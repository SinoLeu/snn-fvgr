...
class CUB():
    def __init__(self, root, is_train=True, data_len=None, transform=None):
        ...
        if self.is_train:
            ...
            self.train_img = [np.array(Image.open(os.path.join(self.root, 'images', train_file)).convert('RGB')) 
...
class CarsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(self.car_annotations[idx])
        car_class = torch.from_numpy(np.array(car_class.astype(np.float32))).long() - 1
        # assert car_class < 196
        assert car_class < 196, f"car_class value is {car_class}. It should be less than 196."
