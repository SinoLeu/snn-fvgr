...
# from pim.pim_module import FPN, WeaklySelector, GCNCombiner
# import torch
# import torch.nn as nn
# import torch.nn.functional as F
# from torchvision.models.feature_extraction import create_feature_extractor
...
## SharedPluginModule
class SharedPluginMoodel(nn.Module):
    def __init__(self, 
                 backbone: torch.nn.Module,
                 return_nodes: Union[dict, None],
                 img_size: int,
                 ...
                 use_combiner: bool,
                 comb_proj_size: Union[int, None],
                 fpn: Union[FPN, None] = None,  # 新增：传入已有的 FPN 实例
                 selector: Union[WeaklySelector, None] = None,  # 新增：传入已有的 WeaklySelector 实例
                 combiner: Union[GCNCombiner, None] = None):  # 新增：传入已有的 GCNCombiner 实例
        super(SharedPluginMoodel, self).__init__()
        ...
        if not use_fpn and (not use_selection and not use_combiner):
            for name in outs:
                ...
                if len(fs_size) == 3:
                    ...
                    # raise ValueError("The size of output dimension must be 3 or 4.")
                    pass
            self.classifier = nn.Linear(out_size, num_classes)
        ...
        self.use_fpn = use_fpn
        if self.use_fpn:
            if fpn is None:  # 如果没有传入 FPN，则创建新的
                self.fpn = FPN(outs, fpn_size, proj_type, upsample_type)
            else:  # 使用传入的 FPN
                self.fpn = fpn
            self.build_fpn_classifier(outs, fpn_size, num_classes)
        ...
        if self.use_selection:
            ...
            if selector is None:  # 如果没有传入 selector，则创建新的
                self.selector = WeaklySelector(outs, num_classes, num_selects, w_fpn_size)
            else:  # 使用传入的 selector
        ...
        if self.use_combiner:
            ...
            if combiner is None:  # 如果没有传入 combiner，则创建新的
                self.combiner = GCNCombiner(total_num_selects, num_classes, gcn_inputs, gcn_proj_size, self.fpn_size)
            else:  # 使用传入的 combiner
